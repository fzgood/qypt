		...
		<div class="row">
			<div class="col-sm-3 col-md-2 sidebar">
				...
				<ul class="nav nav-sidebar">
					...
					<li class="active"><a
						href="${pageContext.request.contextPath}/teacher/index">老师信息</a>
					</li>
			...
			<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
				...
				<form action="${pageContext.request.contextPath}/teacher/add"
					...
					</select> 请输入老师名称：<input id="teacherName" name="teacherName" type="text" />
					<input type="submit" value="commit"> ${error }
				</form>
				<form action="${pageContext.request.contextPath}/teacher/index">
					...
					</select> 老师名字: <input type="text" name="teacherName"
						value="${teacherCustom.teacherName }" /> <input type="submit"
						value="查询">
				</form>
				<div class="table-responsive">
					<table class="table table-striped">
						...
						<tbody>
							<c:forEach items="${teacherAll}" var="t">
								<tr>
									<td width="20%">${t.department.departmentName }</td>
									<td>${t.teacherId }</td>
									<td>${t.teacherName }</td>
								</tr>
							</c:forEach>
					...
					<c:if test="${teacherCustom.pageCurrrnt ne 1 }">
						<a
							href="${pageContext.request.contextPath}/teacher/index?departmentId=${teacherCustom.departmentId}&teacherName=${teacherCustom.teacherName}&pageCurrrnt=${teacherCustom.pageCurrrnt-1}">上一页</a>
					</c:if>
					当前第 ${teacherCustom.pageCurrrnt} 页
					<c:if test="${teacherCustom.pageCurrrnt ne teacherCustom.pageNum }">
						<a
							href="${pageContext.request.contextPath}/teacher/index?departmentId=${teacherCustom.departmentId}&teacherName=${teacherCustom.teacherName}&pageCurrrnt=${teacherCustom.pageCurrrnt+1}">下一页</a>
					</c:if>
				</div>
			</div>
		</div>
